...
|writebackConfiguration|Boolean|Specifies whether or not a group is configured to write back properties on on-premises Active Directory. By default all Security Groups are not writeback enabled. If M365 groupSetting "NewUnifiedGroupWritebackDefault" is to true, by default, M365 groups are writeback enabled. If M365 groupSetting "NewUnifiedGroupWritebackDefault" set to false, by default, M365 groups are writeback not enabled.|
